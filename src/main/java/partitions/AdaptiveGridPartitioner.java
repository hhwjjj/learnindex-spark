...
public class AdaptiveGridPartitioner implements Serializable {
    int numPartitions;
    ...
    public AdaptiveGridPartitioner(JavaRDD<Point> rawSpatialRDD){
    ...
    public boolean analyze() {
        ...
        this.boundingBox = new Rectangle(boundaryEnvelope);
        return true;
    }
    public  JavaRDD<Point> partitionPoints(int numPartitions) {
        double gridCellCount = numPartitions < this.approximateTotalCount ? numPartitions : this.approximateTotalCount;
        double pointsPerPartition = this.approximateTotalCount / gridCellCount;
        ...
        JavaPairRDD<Integer, Point> pairRDD = this.rawSpatialRDD.mapToPair(new PairFunction<Point, Integer, Point>() {
            ...
            public Tuple2<Integer, Point> call(Point point) throws Exception {
                int partitionId = calculatePartitionId(point, boundingBox, pointsPerPartition);
    ...
    private  int calculatePartitionId(Point point, Rectangle boundingBox, double pointsPerPartition) {
        double partitionWidth = (boundingBox.getTo().getX() - boundingBox.getFrom().getX()) / pointsPerPartition;
        double partitionX = (point.getX() - boundingBox.getFrom().getX()) / partitionWidth;
        return (int) Math.floor(partitionX);
    }
}
